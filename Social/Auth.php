...
class Social_Auth {
    ...
    public static function init( $config )
    {
        ...
        require_once $config["base_path"] .  "Message.php";
        ...
        require_once $config["base_path"] .  DIRECTORY_SEPARATOR . "User" . DIRECTORY_SEPARATOR . "Dao.php";
        ...
        Social_Auth::$message = new Social_Message();

        Social_Auth::$session = new Social_Session();

        if ($config["db"]["enabled"]) {
            Social_Auth::$user_dao = new Social_User_Dao( $config["db"] );
        }

        Social_Auth::checkRequirements();

        if( Social_Message::hasError() ){
            $message = Social_Message::getMessage();
            $code = Social_Message::getCode();
            $previous = Social_Message::getPrevious();
            Social_Logger::error( "Error found! Code: $code, Message: $message" );

            Social_Message::clearMessage();
        } else {
            Social_Logger::info("SocialAuth iitialization completed successfully!");
        }
    }
