...
if ($t == "gallery") { // $t and $a are set in gallery/create.phtml
	...
	if ($a == "create") {
		...
		if (!empty($_POST['gallery_name']) && !empty($_POST['gallery_about'])) {
			$creategalleryArray = array(
					'name' => $_POST['gallery_name'],					
					'description' => $_POST['gallery_about'],					
			);
						
			$gallerycreated = BLS_createGallery($creategalleryArray); // function defined in bls_core.php

			if ($gallerycreated) {				
				$data = array(
						'status' => 200,
						'url' => 'http://' . $_SERVER['SERVER_NAME'] . '/create_gallery',
				);
			}
		} // end of if (!empty($_POST['gallery_name']) && !empty($_POST['gallery_about'])).
